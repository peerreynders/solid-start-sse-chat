...
const keepAlive = suite('KeepAlive');

// --- TESTS ---

keepAlive('Start keep Alive, extend, then fire', () => {
	// Given
	const holder = makeLinkHolder(1000);
	const uut = new KeepAlive(holder.link);
...
keepAlive('Start keep Alive, extend, then stop', () => {
	// Given
	const holder = makeLinkHolder(1000);
	const uut = new KeepAlive(holder.link);
