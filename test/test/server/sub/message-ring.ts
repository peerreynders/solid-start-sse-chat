...
import { MessageRing } from '../../../../src/server/sub/message-ring';
...
messageRing('Initialized and empty. Returns keepAlive', () => {
	...
	// Then
	assert.is(
		uut.countAfter(BASE_TIME, BASE_TIME, MIN_WAIT),
		-1,
		'countAfter did not recommend Welcome request (-1)'
	);
	...
	assert.ok(
		...
		'KeepAlive does not have the provided timestamp'
	);
});
...
messageRing('32 messages within interval. Returns all in Chat', () => {
	...
	assert.is(
		uut.countAfter(now, BASE_TIME - 1, 0),
		-1,
		'countAfter did not recommend Welcome request (-1)'
	);
	assert.is(
		uut.countAfter(now, BASE_TIME, 0),
		31,
		'countAfter did not match (31)a'
	);
	assert.is(
		uut.countAfter(now, BASE_TIME, MIN_WAIT),
		31,
		'countAfter did not match (31)b'
	);
	...
	assert.ok(
		message.timestamp === source[0].timestamp,
		'Chat does not have the expected timestamp'
	);
	assert.ok(
		message.kind === 'chat' && message.messages.length === source.length,
		"Source and messages lengths don't match"
	);
	assert.ok(
		message.kind === 'chat' &&
			message.messages.every((v, i) => v === source[i].messages[0]),
		"Source and messages elements don't match"
	);
});

messageRing(
	'64 messages within interval (requires resize). Returns all in Chat',
	() => {
		// GIVEN
		const uut = new MessageRing(HISTORY_INTERVAL);
		...
		assert.is(
			uut.countAfter(now, BASE_TIME, 0),
			63,
			'countAfter did not match (63)a'
		);
		assert.is(
			uut.countAfter(now, BASE_TIME, MIN_WAIT),
			63,
			'countAfter did not match (63)b'
		);
...
messageRing.only('3 sparse messages of 90 secs', () => {
	...
	assert.is(
		uut.countAfter(now, lastTimeA - 1, 0),
		-1,
		'countAfter should be (-1)(A)'
	);
	...
	assert.ok(
		...
		'Chat does not have the expected timestamp (A)'
	);
	assert.ok(
		message.kind === 'chat' &&
			message.messages.length === current.messages.length,
		"Source and messages lengths don't match (A)"
	);
	assert.ok(
		message.kind === 'chat' &&
			message.messages.every((v, i) => v === current.messages[i]),
		"Source and messages elements don't match (A)"
	);
	...
	assert.ok(
		message.kind === 'keep-alive',
		"Message returned isn't KeepAlive (A)"
	);
	assert.ok(
		message.timestamp === now,
		'KeepAlive does not have the provided timestamp (A)'
	);
	...
	assert.is(
		uut.countAfter(now, lastTimeB, 0),
		0,
		'countAfter should be (0) (B)'
	);
	...
	assert.ok(
		message.kind === 'keep-alive',
		"Message returned isn't KeepAlive (B)"
	);
	assert.ok(
		message.timestamp === now,
		'KeepAlive does not have the provided timestamp (B)'
	);
	...
	assert.ok(
		message.timestamp === current.timestamp,
		'Chat does not have the expected timestamp (C)'
	);
	assert.ok(
		message.kind === 'chat' &&
			message.messages.length === current.messages.length,
		"Source and messages lengths don't match (C)"
	);
	assert.ok(
		message.kind === 'chat' &&
			message.messages.every((v, i) => v === current.messages[i]),
		"Source and messages elements don't match (C)"
	);
	...
	assert.is(
		uut.countAfter(now, lastTimeD - 1, 0),
		-1,
		'countAfter should be (-1) (D)'
	);
	assert.is(
		uut.countAfter(now, lastTimeD, 0),
		1,
		'countAfter should be (1) (D)'
	);
	assert.is(
		uut.countAfter(now, lastTimeDD, 0),
		0,
		'countAfter should be (0) (D)'
	);
	...
	assert.ok(
		message.timestamp === current.timestamp,
		'Chat does not have the expected timestamp (D)'
	);
	assert.ok(
		message.kind === 'chat' &&
			message.messages.length === current.messages.length,
		"Source and messages lengths don't match (D)"
	);
	assert.ok(
		message.kind === 'chat' &&
			message.messages.every((v, i) => v === current.messages[i]),
		"Source and messages elements don't match (D)"
	);
	...
	assert.is(
		uut.countAfter(now, lastTimeD, 0),
		1,
		'countAfter should be (1) still (E)'
	);
	// ... however if there is a minimum wait before toMessage() the message may be gone
	assert.is(
		uut.countAfter(now, lastTimeD, MIN_WAIT),
		-1,
		'countAfter should be (-1) (E)'
	);
});
