...
function makeLinkHolder(time = 0) {
	const holder: {
		...
		link: {
			...
			cancelTimeout: () => void (holder.cancelTimeoutCount += 1),
		},
	};
	return holder;
}
...
longPoller('Basic flow', async () => {
	...
	assert.is(uut.isActive(), false, 'Longpoller is unexpectedly active');
	...
	assert.is(uut.isActive(), true, 'Longpoller is unexpectedly inactive');
	...
	// disconnect() should have: abort() & cancelTimeout();
	assert.is(holder.abortCount, 1, 'disconnect failed to abort (3)');
	assert.is(
		holder.cancelTimeoutCount,
		1,
		'disconnect failed to cancel timeout (3)'
	);
	...
	assert.is(uut.isActive(), false, 'Longpoller is unexpectedly active (3)');
	assert.type(holder.fn, 'undefined', 'fetch unexpectedly scheduled (3)');
});

longPoller('Basic cancel timeout flow', async () => {
	// Given
	const holder = makeLinkHolder(1000);
	const uut = new Longpoller(holder.link);
	...
	// ---
	// Get ready to observe: fetch (2) being ABORTED as part of a timeout
	holder.fetchDo = false; // simulate abort during fetch
	holder.fetchHref = undefined;
	holder.abortCount = 0;
	holder.cancelTimeoutCount = 0;
	...
	assert.is(
		holder.cancelTimeoutCount,
		1,
		'disconnect failed to cancel timeout (2)'
	);
	...
	assert.is(uut.isActive(), true, 'Longpoller is unexpectedly inactive (2)');
	assert.not.type(
		holder.fn,
		'undefined',
...
longPoller('Poll failure', async () => {
	...
	assert.is(holder.cancelTimeoutCount, 1, 'failed to call disconnect (2)');
